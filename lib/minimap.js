...
/**
 * Arrastrar el minimapa por su cabecera para colocarlo donde no estorbe.
 *
 * Un popover no se puede mover: el SDK solo sabe abrirlo y cerrarlo. Asi que
 * se mide cuanto has arrastrado y al soltar se le pide al script de fondo que
 * lo vuelva a abrir en el sitio nuevo. Por eso no hay arrastre en vivo: el
 * panel salta al soltarlo. Las coordenadas van en `screenX/screenY`, que son
 * de la pantalla y siguen valiendo aunque el puntero salga del iframe.
 */
function wireDrag() {
  const bar = root.querySelector(".mm-toolbar");
  if (!bar) return;
  let drag = null;

  bar.addEventListener("pointerdown", ev => {
    if (ev.button !== 0 || ev.target.closest(".mm-btn")) return;
    ev.preventDefault();
  ...
  bar.addEventListener("pointermove", ev => {
    ...
    if (Math.abs(ev.screenX - drag.x) > 3 || Math.abs(ev.screenY - drag.y) > 3) {
      drag.moved = true;
      root.classList.add("mm-moving");
    }
  });
  ...
  const drop = ev => {
    ...
    drag = null;
    root.classList.remove("mm-moving");
    if (moved) send({ type: "move", dx: ev.screenX - x, dy: ev.screenY - y });
  };
