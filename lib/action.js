  ...
  ui.innerHTML = `
    ...
    <div class="gm-only" ${gm ? "" : "hidden"}>
      ...
      <div class="sub ${draft.mode === "image" ? "" : "off"}" id="imgSub">
        ${sharedMaps()}
        <div class="pick">
          <input type="text" id="src" value="${esc(draft.src)}" placeholder="URL de la imagen">
...
function wire() {
  ...
  on("dock", "change", e => { setPrefs({ dock: e.target.value }); if (prefs().open) refreshPanel(); });
  ...
  on("pick", "click", async () => {
    // La biblioteca de imagenes de Owlbear, en vez de pedir una URL a mano.
