...
import { prefs, setPrefs, send, onMessage } from "./prefs.js";
...
const fileName = u => {
  try { return decodeURIComponent(u.split("/").pop().split("?")[0]).slice(0, 40); }
  catch { return u.slice(-40); }
};

const option = (value, label) =>
  `<option value="${esc(value)}" ${value === draft.src ? "selected" : ""}>${esc(label)}</option>`;

/**
 * Imágenes que se pueden elegir sin pasar por la biblioteca.
 *
 * Dos grupos: los mapas que ya tienen revelado en esta sala (elegir uno es
 * compartir el revelado con las escenas que lo usan) y las imágenes de la capa
 * de mapa de la escena abierta, que sirven de plano sin tener que subir nada.
 */
function imageOptions() {
  ...
  for (const i of store.items) {
    if (i.type !== "IMAGE" || i.layer !== "MAP" || !i.image?.url) continue;
    if (seen.has(i.image.url)) continue;
    seen.add(i.image.url);
    inScene.push(i);
  }

  if (!used.length && !inScene.length) return "";
  return `<select id="shared">
    <option value="">— o elegir una imagen ya conocida —</option>
    ${used.length ? `<optgroup label="Mapas ya en uso (comparten revelado)">
      ${used.map(u => option(u, fileName(u))).join("")}</optgroup>` : ""}
    ${inScene.length ? `<optgroup label="Imágenes de esta escena">
      ${inScene.map(i => option(i.image.url, i.name || fileName(i.image.url))).join("")}</optgroup>` : ""}
  </select>`;
}
...
function wire() {
  ...
  // Lo abre el script de fondo: al aparecer la biblioteca, Owlbear cierra
  // este panel, y la respuesta ya no tendria a donde volver. El fondo la
  // recibe y la aplica directamente a la escena.
  on("pick", "click", () => send({ type: "pickImage" }));
  on("save", "click", async () => {
...
OBR.onReady(async () => {
  ...
  // Si el panel sobrevive a la biblioteca, se rellena el campo al elegir.
  onMessage(msg => {
    if (msg.type === "picked" && draft) {
      draft.mode = "image";
      draft.src = msg.src;
      render();
    }
